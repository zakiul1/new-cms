@php
    ...
    $aClass = 'text-slate-700 hover:text-slate-900 hover:underline underline-offset-4';
    if (!empty($singleLine)) {
...
<style>
  .linksblock-grid { grid-template-columns: repeat(var(--lb-cols-mobile, 2), minmax(0, 1fr)); }
  @media (min-width: 768px) { .linksblock-grid { grid-template-columns: repeat(var(--lb-cols-md, 3), minmax(0, 1fr)); } }
  @media (min-width: 1024px){ .linksblock-grid { grid-template-columns: repeat(var(--lb-cols-lg, 4), minmax(0, 1fr)); } }
</style>
...
<div class="page-container my-6">
    @if (!empty($hide))
        ...
        <button
            type="button"
            class="inline-flex items-center justify-center w-10 h-10 cursor-pointer border-slate-200"
            aria-controls="{{ $uid }}"
            aria-expanded="false"
            onclick="
        ...
        >
            ...
            <svg data-icon="down" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-slate-700" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M6 9l6 6 6-6"/>
            </svg>

            {{-- Arrow UP (hidden by default, shown when open) --}}
            <svg data-icon="up" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-slate-700 hidden" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
